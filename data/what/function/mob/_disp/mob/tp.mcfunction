$tag @n[tag=what.ai,scores={what.id=$(sid)},distance=0..3] add what.cur

execute unless entity @n[tag=what.cur,distance=0..3] run function what:mob/_disp/mob/tp_furthest with entity @s item.components.minecraft:custom_data
execute at @n[tag=what.cur,distance=0..3] run tp @s ~ ~.5 ~ facing ^ ^ ^1
#say 1
...
tag @n[tag=what.cur,distance=0..3] remove what.cur
