...
execute at @a if dimension what:backrooms as @e[distance=0..120, type=item_display, tag=what.block_placed, tag=!what.no_chunking,tag=!falling,  sort=random,limit=36] at @s run function what:block/chunk/check
